from Employee_filds_class import Employee2
...
# ID компании отсутствует в запросе
...
def test_employee_changes_applying():
    employ.empoyee_change(employee_id, change_data, token)
    ...
    assert employee[0]['isActive'] is False

# Отсутствует id сотрудника
# ПРОЙДЕН: Метод вернул ошибку
def test_employee_changes_no_id():
    result = employ.empoyee_change(None, change_data, token)
    assert result[1] != 200
...
def test_employee_changes_no_token():
    result = employ.empoyee_change(employee_id, change_data, None)
    assert result[1] != 200
